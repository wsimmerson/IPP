...
def group_collide(group, other):
    remove_set = set([])
    for sprite in group:
        if sprite.collide(other) == True:
            remove_set.add(sprite)
            explosions.add(Sprite(sprite.get_pos(), sprite.vel, 0, 0, explosion_image, explosion_info, explosion_sound))
...
def draw(canvas):
    global time, started, lives, score, rocks, missiles, explosions, soundtrack
...
def rock_spawner():
    """
        Timer handler which spawn randomized astroids
    """
    global rocks
    if len(rocks) < 12 and started:
        ...
        rocks.add(Sprite(random.choice(pos), vel, ang, ang_vel, asteroid_image, asteroid_info))
